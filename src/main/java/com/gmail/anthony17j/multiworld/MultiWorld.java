...
public class MultiWorld implements ModInitializer {
    ...
    public static final String creativeWorldName = "creative";
    ...
    public void onInitialize() {
        ...
        ServerLifecycleEvents.SERVER_STARTED.register((MinecraftServer server) -> {
            createWorld(
                    server,
                    creativeWorldName,
                    RegistryKey.of(RegistryKeys.DIMENSION_TYPE, Identifier.of(namespace, creativeWorldName)),
                    new VoidChunkGenerator(server.getRegistryManager().getOrThrow(RegistryKeys.BIOME)),
                    BlockPos.ORIGIN
            );
    ...
    private static void createWorldWithDimensions(MinecraftServer server, String worldString, long seed) {
        ...
        // createWorld(server, worldString, DimensionTypes.OVERWORLD, new NoiseChunkGenerator(MultiNoiseBiomeSource.create(multiNoisePresetLookup.getOrThrow(MultiNoiseBiomeSourceParameterLists.OVERWORLD)), chunkGeneratorSettingsLookup.getOrThrow(ChunkGeneratorSettings.OVERWORLD)), seed, null);
        // createWorld(server, worldString + "_nether", DimensionTypes.THE_NETHER, new NoiseChunkGenerator(MultiNoiseBiomeSource.create(multiNoisePresetLookup.getOrThrow(MultiNoiseBiomeSourceParameterLists.NETHER)), chunkGeneratorSettingsLookup.getOrThrow(ChunkGeneratorSettings.NETHER)), seed, null);
        // createWorld(server, worldString + "_end", DimensionTypes.THE_END, new NoiseChunkGenerator(TheEndBiomeSource.createVanilla(biomeLookup), chunkGeneratorSettingsLookup.getOrThrow(ChunkGeneratorSettings.END)), seed, null);

        createWorld(server, worldString, DimensionTypes.OVERWORLD, server.getOverworld().getChunkManager().getChunkGenerator(), seed, null);
        createWorld(server, worldString + "_nether", DimensionTypes.THE_NETHER, server.getWorld(World.NETHER).getChunkManager().getChunkGenerator(), seed, null);
        createWorld(server, worldString + "_end", DimensionTypes.THE_END, server.getWorld(World.END).getChunkManager().getChunkGenerator(), seed, null);
    }

    private static void createWorld(MinecraftServer server, String worldString, RegistryKey<DimensionType> dimensionType, ChunkGenerator generator, BlockPos spawnPos) {
        createWorld(server, worldString, dimensionType, generator, server.getOverworld().getSeed(), spawnPos);
    }

    private static void createWorld(MinecraftServer server, String worldString, RegistryKey<DimensionType> dimensionType, ChunkGenerator generator, long seed, BlockPos spawnPos) {
        ...
        LevelProperties newLevelProperties = new LevelProperties(
